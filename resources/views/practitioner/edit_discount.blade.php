                        ...
                        <form method="POST" action="{{route('update_discount', $discount->id)}}">
                            ...
                            <div class="mb-4">
                                ...
                                <select id="apply_to" class="form-select apply_to" name="apply_to">
                                    <option value="all" {{$discount->apply_to === 'all'? 'selected': ''}}>All
                                        Offerings
                                    </option>
                                    <option value="specific" {{$discount->apply_to === 'specific'? 'selected': ''}}>
                                        Specific Offerings
                                    </option>
                                </select>
                            </div>
                            <div class="mb-4 select2-div offerings-select-container"
                                 style="{{$discount->apply_to === 'specific'? '': 'display: none;'}}">
                                <?php
                                ...
                                <select id="offerings" multiple class="form-control select2" name="offerings[]">
                                    ...
                                    @foreach($offerings as $offering)
                                        <option
                                            value="{{ $offering->id }}" {{ in_array($offering->id, $selectedOfferings) ? 'selected' : '' }}>
                                            {{ $offering->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col mb-4">
                                ...
                                <input type="number" class="form-control" value="{{$discount->coupon_amount}}"
                                       placeholder="0" name="coupon_amount" id="coupon_amount">
                            </div>
